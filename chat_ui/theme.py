...
class Colors:
    ...
    def hex_to_list(hex_color: str) -> List[float]:
        ...
        rgba_list = list(get_color_from_hex(hex_color))
        logger.info("color_conversion", hex_color=hex_color, rgba_list=rgba_list)
        return rgba_list
...
# Log theme initialization on module load (avoid circular imports)
def _log_theme_initialization() -> None:
    """Log theme initialization after module is fully loaded."""
    try:
        logger.info("theme_initialized",
                   bubble_radius=Sizes.BUBBLE_RADIUS,
                   header_height=Sizes.HEADER_HEIGHT,
                   primary_color=Colors.PRIMARY_BLUE_HEX,
                   spacing_medium=Spacing.MEDIUM)
    except Exception:
        # Fallback if logging fails during initialization
        pass

# Call after all class definitions
_log_theme_initialization()
